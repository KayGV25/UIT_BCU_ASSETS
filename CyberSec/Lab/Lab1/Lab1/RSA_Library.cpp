...
void generate_rsa_keys(int bits, const char *public_key_file, const char *private_key_file) {
    RSA *rsa = RSA_new();
    BIGNUM *bn = BN_new();
    BN_set_word(bn, RSA_F4);  // Common exponent 65537
...
int rsa_encrypt(const char *public_key_file, const char *input, const char *output) {
...
int rsa_decrypt(const char *private_key_file, const char *input_file, const char *output_file) {
