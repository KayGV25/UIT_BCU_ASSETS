...
### **On Linux**
```bash
g++ AES_CommandLine.cpp -o aes_cpp -lcrypto -lssl
```

### **On Windows (Visual Studio)**
```bash
cl AES_CommandLine.cpp /I"C:\OpenSSL\include" /link /LIBPATH:"C:\OpenSSL\lib" libcrypto.lib libssl.lib
```
...
2. **Encrypt a File:**
   ```bash
   ./aes_cpp cbc encrypt <key_in_hex> <iv_in_hex> input.txt encrypted.bin
   ```

3. **Decrypt a File:**
   ```bash
   ./aes_cpp ctr decrypt <key_in_hex> <iv_in_hex> encrypted.bin decrypted.txt
   ```

---

This comprehensive guide walks students through the process of implementing AES encryption and decryption with OpenSSL in C++. It supports multiple modes and offers detailed comments to aid understanding.
